...
from __data__.joinquant.__tables__.asharebalancesheet.config import main as config
...
class main(meta, type('config', (), config.params)):
    ...
    def __source__(self, id_key):
        df = jq.finance.run_query(jq.query(jq.finance.STK_BALANCE_SHEET).order_by(jq.finance.STK_BALANCE_SHEET.id).filter((jq.finance.STK_BALANCE_SHEET.source_id == 321003) & (jq.finance.STK_BALANCE_SHEET.report_type == 0) & (jq.finance.STK_BALANCE_SHEET.id > id_key)).limit(5000))
