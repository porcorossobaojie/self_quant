...
from __data__.joinquant.__tables__.ashareincome.config import main as config
...
class main(meta, type('config', (), config.params)):
    ...
    def __source__(self, id_key):
        df = jq.finance.run_query(jq.query(jq.finance.STK_INCOME_STATEMENT).order_by(jq.finance.STK_INCOME_STATEMENT.id).filter((jq.finance.STK_INCOME_STATEMENT.source_id == 321003) & (jq.finance.STK_INCOME_STATEMENT.report_type == 0) & (jq.finance.STK_INCOME_STATEMENT.id > id_key)).limit(5000))
