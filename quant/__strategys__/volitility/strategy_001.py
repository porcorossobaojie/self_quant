...
if __name__ == '__main__':
    ...
    turnover = fac_obj.turnover.rolling(21).mean()
    neu_obj = fac001.stats.neutral(neu_axis=1, fac=turnover)
    fac = fac001.copy()
    fac = fac_obj.__filter__(fac)
    fac = fac.rollings(21).min(3).mean()
    fac = fac.loc[:, [i[:3] not in ['002', '300', '688', '301'] for i in fac.columns]]
    #fac = neu_obj.resid + turnover.mul(neu_obj.params['fac'], axis=0) * 0.5
    count = 24
